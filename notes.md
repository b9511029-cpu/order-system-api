...
#  解決 資料庫 分散問題 紀錄
因為建立多個 API ，分散式的資料庫無法統一管理，因此需要將所有API 資料庫 集中連到共享的資料庫 app.db
為了讓資料庫形成一個 關聯設計(relational design),主要因為分散式的DB 無法建立關聯，會難以管理
所以要建立一個database 相關架構 將.db/conn/schema.sql 獨立出來 
